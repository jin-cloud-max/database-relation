...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!findProducts.length) {
      // como ele retorna um array, pra ver se possui algum valor a gente olha pelo comprimento
      throw new AppError('Products not found');
    }

    // mostra qual produto não foi encontrado dentro do array
    const findProductsId = findProducts.map(product => product.id);

    const checkInesxistentProducts = products.filter(
      product => !findProductsId.includes(product.id),
    );

    if (checkInesxistentProducts.length) {
      // como ele retorna um array, pra ver se possui algum valor a gente olha pelo comprimento
      throw new AppError(
        `Could not find product ${checkInesxistentProducts[0].id}`,
      );
    }
